...
async function isVaultUnlocked() {
  ...
  if (!lazarus_lock) return true; // no PIN set
...
browser.runtime.onMessage.addListener(msg => {
  ...
  if (msg.action === "toggleCommandPalette" && typeof toggleCommandPalette === 'function') {
...
(async function init() {
    ...
    (function mobileFriendlyInFieldUI() {
      ...
      async function showDropdown(field) {
        ...
        try {
          ...
          const relevant = entries.filter(e => e.fieldName === identifier);
          if (!relevant.length) {
  ...
  if (!isBlacklisted && cachedSettings.searchIndexing) {
