...
const SEARCH_ENGINES = [
  'google.com', 'www.google.com', 'google.co.uk', 'google.de', 'google.fr',
...
function isSearchEngine() {
  return SEARCH_ENGINES.includes(currentHostname);
}
...
let commandPaletteToggleFn = null;
...
browser.storage.onChanged.addListener((changes, area) => {
  if (area === 'local' && changes.lazarus_settings) {
    const old = cachedSettings.savePasswords;
    cachedSettings = { ...cachedSettings, ...(changes.lazarus_settings.newValue || {}) };
    if (old !== cachedSettings.savePasswords && !isBlacklisted) {
      attach(document);
    }
  }
});
...
async function isVaultUnlocked() {
  const { lazarus_lock, lazarus_session_unlocked } = await browser.storage.local.get(["lazarus_lock", "lazarus_session_unlocked"]);
  if (!lazarus_lock) return true;
...
function isSensitive(field) {
  ...
  if (type === 'password' && !cachedSettings.savePasswords) return true;
...
function getFieldIdentifier(el) {
  if (el.name) {
    ...
    let selector = `${tag}[name="${escapedName}"]`;
    if (tag === 'input' && el.type) {
      selector += `[type="${CSS.escape(el.type.toLowerCase())}"]`;
    }
    return selector;
  }
  if (el.id) {
...
function findElementAcrossShadows(selector) {
  try {
  ...
  function searchInRoot(root) {
    ...
    for (const elem of elements) {
      if (elem.shadowRoot) {
        const found = searchInRoot(elem.shadowRoot);
...
function attach(root) {
  ...
  let selector = 'input:not([type="hidden"]):not([type="submit"]):not([type="checkbox"]):not([type="radio"])';
  if (!cachedSettings.savePasswords) selector += ':not([type="password"])';
...
function detachAll() {
  ...
  if (iconHost) iconHost.remove();
  const dropdownHost = document.getElementById('lazarus-dropdown-host');
  if (dropdownHost) dropdownHost.remove();
  const cpHost = document.getElementById('lazarus-cp-host');
  if (cpHost) cpHost.remove();
}

function findFieldByIdentifier(identifier) {
  try { return findElementAcrossShadows(identifier); } catch { return null; }
}

function nativeSetValue(field, text) {
...
function restoreTextDirect(identifier, text) {
  ...
  console.log('[Lazarus] restoreTextDirect:', { identifier, text });
  function attemptRestoration(retriesLeft) {
    let field = findFieldByIdentifier(identifier);
    if (field) {
      console.log('[Lazarus] Found field with exact identifier:', identifier);
      simulateUserTyping(field, text);
      return;
    }

    const nameMatch = identifier.match(/\[name="([^"]+)"\]/);
    const typeMatch = identifier.match(/\[type="([^"]+)"\]/);
    if (nameMatch) {
      const name = nameMatch[1];
      if (typeMatch) {
        const type = typeMatch[1];
        const selector = `[name="${CSS.escape(name)}"][type="${CSS.escape(type)}"]`;
        field = findElementAcrossShadows(selector);
        if (field) {
          console.log('[Lazarus] Found field with name+type fallback:', selector);
          simulateUserTyping(field, text);
          return;
        }
      }
      ...
      field = findElementAcrossShadows(selector);
      if (field) {
        console.log('[Lazarus] Found field with name-only fallback:', selector);
        simulateUserTyping(field, text);
        return;
      }
    }
    ...
    if (idMatch) {
      ...
      if (field) {
        console.log('[Lazarus] Found field with id fallback:', id);
        simulateUserTyping(field, text);
        return;
      }
    }
    ...
    if (retriesLeft > 0) {
      console.log(`[Lazarus] Field not found, retrying... (${retriesLeft} retries left)`);
      setTimeout(() => attemptRestoration(retriesLeft - 1), 600);
    } else {
      console.warn('[Lazarus] Could not find field for identifier:', identifier);
    }
  }
  attemptRestoration(5);
}
...
browser.runtime.onMessage.addListener(msg => {
  ...
  if (msg.action === "toggleCommandPalette") {
    if (typeof commandPaletteToggleFn === 'function') {
      commandPaletteToggleFn();
    }
  }
});

function initInFieldUI() {
  if (document.getElementById('lazarus-icon-host')) return;
  ...
  const shadowRoot = iconHost.attachShadow({ mode: 'closed' });
  ...
  const dropShadow = dropdownHost.attachShadow({ mode: 'closed' });
  ...
  async function showDropdown(field) {
    ...
    try {
      ...
      if (entries && entries.iv) {
        ...
        const iv = new Uint8Array(entries.iv);
        const ciphertext = new Uint8Array(entries.data);
      ...
      if (entries && Array.isArray(entries)) {
        ...
        else if (entries.length) entries = [entries[0]];
        else entries = [];
      } else {
        entries = [];
      }
...
function initCommandPalette() {
  if (document.getElementById('lazarus-cp-host')) return;
  ...
  async function show() {
    if (isBlacklisted) return;
    if (!cachedSettings.searchIndexing) return;
  ...
  commandPaletteToggleFn = function() {
    if (cpVisible) hide();
    else show();
  };
    ...
    if (msg.action === 'blacklist-changed') {
      refreshBlacklist().then(() => {
        if (isBlacklisted) {
          ...
          attach(document);
          initInFieldUI();
          if (cachedSettings.searchIndexing) initCommandPalette();
        }
      });
    }
  });
