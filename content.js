...
function getFieldIdentifier(el) {
  ...
  if (el.name) {
    ...
    const matches = document.querySelectorAll(`[name="${escapedName}"]`);
    if (matches.length === 1) {
...
function findFieldByIdentifier(identifier) {
  try { return document.querySelector(identifier); } catch { return null; }
}
...
function initInFieldUI() {
  ...
  async function showDropdown(field) {
    ...
    try {
      ...
      // entries are already filtered by fieldName from background
      const relevant = entries; // since background filtered by fieldName
      if (!relevant.length) {
