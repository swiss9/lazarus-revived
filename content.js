...
(async function init() {
  ...
  if (!isBlacklisted && cachedSettings.searchIndexing) {
    (function commandPalette() {
      ...
      async function show() {
        if (isBlacklisted) return;
        if (cachedSettings.restoreRequiresPassword && await isVaultLocked()) {
        ...
        if (await isVaultLocked() && !cachedSettings.restoreRequiresPassword) {
          results.innerHTML = '<div class="locked">🔒 Vault is locked</div>';
          host.style.display = 'flex';
          cpVisible = true;
          input.style.display = 'none';
        ...
        if (cachedSettings.restoreRequiresPassword && !(await isVaultLocked())) {
          // vault is not locked, so restoreRequiresPassword is satisfied by not having a lock set; allow.
        }
        input.style.display = '';
        host.style.display = 'flex';
        cpVisible = true;
