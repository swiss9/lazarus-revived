...
async function deriveEncryptionKey(pin, salt) {
  if (!salt) return null;
  const rawKey = await deriveKey(pin, salt, 'encrypt');
...
async function encryptBlob(entries, key) {
  if (!key) return null;
...
async function saveBlobToBackground(blob) {
  const resp = await browser.runtime.sendMessage({ action: "saveBlob", blob });
  if (!resp || !resp.success) throw new Error('Failed to save blob to background');
}
...
async function loadEntries(filterUrl) {
  try {
    ...
    if (stored.iv) {
      if (!encryptionKey) return [];
      const decrypted = await decryptBlob(stored, encryptionKey);
      return decrypted || [];
    }
    return stored;
  } catch (e) {
...
async function saveEntries(entries) {
  try {
    if (lazarus_lock_active && encryptionKey) {
      const encrypted = await encryptBlob(entries, encryptionKey);
...
async function setLockPin(pin) {
  try {
    ...
    const newKey = await deriveEncryptionKey(pin, encSalt);
    if (!newKey) throw new Error('Failed to derive encryption key');

    const stored = await loadBlobFromBackground(null);
    let entries = [];
    if (stored) {
      if (stored.iv) {
        if (!encryptionKey) throw new Error('Old encryption key missing. Please unlock first.');
        entries = await decryptBlob(stored, encryptionKey);
        if (!entries) throw new Error('Failed to decrypt existing data.');
      } else {
        entries = stored;
      }
    }

    const encrypted = await encryptBlob(entries, newKey);
    if (!encrypted) throw new Error('Failed to encrypt data.');

    const keyResponse = await browser.runtime.sendMessage({ action: "setKey", key: await exportKeyStringWithKey(newKey) });
    if (!keyResponse || !keyResponse.success) throw new Error('Failed to set encryption key in background');

    // Store key in session storage before committing lock
    encryptionKey = newKey;
    await storeEncryptionKeyInSession();

    // Commit lock and set session_unlocked flag atomically via background
    const lockResponse = await browser.runtime.sendMessage({
      action: "atomicSetLock",
      data: {
        lazarus_lock: true,
    ...
    if (!lockResponse || !lockResponse.success) {
      // Rollback: clear session key
      await browser.storage.session.set({ lazarus_encryption_key: null });
      await browser.runtime.sendMessage({ action: "clearKey" });
      throw new Error('Failed to save lock state to storage');
    }
...
async function exportKeyStringWithKey(key) {
  if (!key) return null;
...
async function clearLock(pin) {
  try {
    ...
    if (!lazarus_enc_salt) throw new Error('Encryption salt missing');

    const stored = await loadBlobFromBackground(null);
    let entries = stored;

    if (stored && stored.iv) {
      const key = await deriveEncryptionKey(pin, lazarus_enc_salt);
      if (!key) throw new Error('Failed to derive encryption key');
      entries = await decryptBlob(stored, key);
      if (!entries) throw new Error('Failed to decrypt data');
    } else if (!stored) {
      entries = [];
    }

    const response = await browser.runtime.sendMessage({
      action: "atomicClearLock",
      data: { entries }
    });
    if (!response || !response.success) throw new Error('Failed to clear lock state in storage');
    ...
    showToast('Failed to delete PIN: ' + e.message);
    return false;
  }
}
...
async function ensureRestorePermission() {
  ...
  const { lazarus_session_unlocked } = await browser.storage.local.get("lazarus_session_unlocked");
...
async function resurrectFullForm() {
  ...
  if (!(await ensureRestorePermission())) return;
  try {
    const entries = await loadEntries(currentTabUrl);
...
function showPinModal(mode, callback) {
  ...
  if (mode === 'set') {
    title = 'Set PIN (min 6 characters)';
    showConfirm = true;
  ...
  function submit() {
    ...
    if (mode === 'set' || mode === 'change') {
      if (pin.length < 6) {
        errorDiv.textContent = 'PIN must be at least 6 characters.';
    ...
    if (mode === 'set') {
      ...
      (async () => {
        try {
          ...
          const oldKey = await deriveEncryptionKey(current, lazarus_enc_salt);
          if (!oldKey) {
      ...
      verifyPin(pin).then(valid => {
        if (valid) {
          deriveEncryptionKey(pin, lazarus_enc_salt).then(async (key) => {
            if (!key) {
              errorDiv.textContent = 'Failed to derive encryption key.';
              errorDiv.style.display = 'block';
              if (callback) callback(false);
            ...
            encryptionKey = key;
            await storeEncryptionKeyInSession();
            const keyResp = await browser.runtime.sendMessage({ action: "setKey", key: await exportKeyString() });
            if (!keyResp || !keyResp.success) {
              errorDiv.textContent = 'Failed to set key in background.';
              errorDiv.style.display = 'block';
              if (callback) callback(false);
              return;
            }
            // Set session_unlocked flag in local storage via background
            await browser.runtime.sendMessage({
              action: "atomicSetLock",
              data: { lazarus_lock: false, encryptedBlob: null } // dummy, but we need to set flag
            });
            // Actually we should not call atomicSetLock with lock false; we need a dedicated flag setter.
            // We'll just set it directly in local storage.
            await browser.storage.local.set({ lazarus_session_unlocked: true });
            lazarus_unlocked = true;
            updateLockButtonText();
            applySettingsToUI();
            lockScreen.style.display = 'none';
  ...
  try {
    ...
    if (tab?.url) {
      try {
        ...
        currentTabUrl = url.origin + url.pathname;
      } catch { currentTabUrl = tab.url; }
    }
    await loadBlacklist();
    await loadLockState();
    await loadSettings();
    ...
    // Read session_unlocked from local storage
    const { lazarus_session_unlocked } = await browser.storage.local.get("lazarus_session_unlocked");
      ...
      enqueue(async () => {
        if (lazarus_lock_active) {
          if (lazarus_unlocked) {
            ...
            encryptionKey = null;
            await browser.storage.session.set({ lazarus_encryption_key: null });
            await browser.storage.local.set({ lazarus_session_unlocked: false });
            await browser.runtime.sendMessage({ action: "clearKey" });
            updateLockButtonText();
            applySettingsToUI();
            lockScreen.style.display = 'flex';
    ...
    $('unlock-btn')?.addEventListener('click', async () => {
      ...
      if (await verifyPin(pin)) {
        encryptionKey = await deriveEncryptionKey(pin, lazarus_enc_salt);
        if (!encryptionKey) {
        ...
        const keyResp = await browser.runtime.sendMessage({ action: "setKey", key: await exportKeyString() });
        if (!keyResp || !keyResp.success) {
          pinError.textContent = 'Failed to set key in background.';
          pinError.style.display = 'block';
          return;
        }
        await browser.storage.local.set({ lazarus_session_unlocked: true });
