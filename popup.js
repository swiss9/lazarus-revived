...
async function refreshEntries() {
  ...
  if (!entries || !entries.length) {
    container.innerHTML = `<div class="empty-state"><div class="empty-icon">📜</div><div class="empty-title">No drafts saved ${showAll ? 'yet' : 'for this site'}</div><div class="empty-desc">Type in any form field and Lazarus will automatically capture your text here.</div></div>`;
    return;
  }
...
async function init() {
  browser.runtime.onMessage.addListener((msg) => { if (msg.action === 'blacklist-changed') { loadBlacklist().then(() => { updateStatusBadge(); updateBlacklistButton(); }); } });
  ...
  updateLockButtonText();
  if (currentTabUrl) { updateStatusBadge(); updateBlacklistButton(); }
