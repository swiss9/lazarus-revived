      ...
      verifyPin(pin).then(valid => {
        if (valid) {
          if (mode === 'unlock-session') {
            // Just unlock the session, do not disable the lock
            lazarus_unlocked = true;
            browser.storage.session.set({ lazarus_session_unlocked: true });
            ...
            // Normal verify (could be from lock-screen or restore permission)
            if (callback) {
              lazarus_unlocked = true;
              document.getElementById('lock-btn').textContent = '🔓 Unlocked';
              browser.storage.session.set({ lazarus_session_unlocked: true });
              ...
              // This case shouldn't happen anymore for the lock button, but keep for safety
              disableLock().then(() => {
                document.getElementById('lock-btn').textContent = '🔓 Unlocked';
...
async function init() {
  ...
  document.getElementById('lock-btn').addEventListener('click', () => {
    if (lazarus_lock_active) {
      // PIN is set – toggle session lock/unlock
      if (lazarus_unlocked) {
        // Session is unlocked – lock it
        lazarus_unlocked = false;
        browser.storage.session.set({ lazarus_session_unlocked: false });
        document.getElementById('lock-btn').textContent = '🔒 Locked';
        refreshEntries();
      } else {
        // Session locked – ask for PIN to unlock session
        showPinModal('unlock-session');
      }
    } else {
      // No PIN – allow setting one
      showPinModal('set');
    }
  });
