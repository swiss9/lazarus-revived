...
async function setLockPin(pin) {
  try {
    ...
    if (!response || !response.success) throw new Error('Failed to save lock state to storage');
    ...
    if (!keyResp || !keyResp.success) throw new Error('Failed to set key in background');
