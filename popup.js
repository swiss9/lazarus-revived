...
async function setLockPin(pin) {
  try {
    ...
    // Set session flag and persist key BEFORE committing the lock
    ...
    if (!lockResponse || !lockResponse.success) {
      // Rollback: clear session flag and key
      await browser.storage.session.set({ lazarus_session_unlocked: false });
      await browser.storage.session.set({ lazarus_encryption_key: null });
      const clearResp = await browser.runtime.sendMessage({ action: "clearKey" });
      if (!clearResp || !clearResp.success) {
        console.warn('[Lazarus] Failed to clear key from background after atomicSetLock failure.');
      }
...
async function clearLock(pin) {
  try {
    ...
    const clearResp = await browser.runtime.sendMessage({ action: "clearKey" });
    if (!clearResp || !clearResp.success) {
      console.warn('[Lazarus] Failed to clear key from background after lock clearance.');
    }
      ...
      enqueue(async () => {
        if (lazarus_lock_active) {
          if (lazarus_unlocked) {
            ...
            const clearResp = await browser.runtime.sendMessage({ action: "clearKey" });
            if (!clearResp || !clearResp.success) {
              console.warn('[Lazarus] Failed to clear key from background after locking.');
            }
            updateLockButtonText();
            applySettingsToUI();
            lockScreen.style.display = 'flex';
